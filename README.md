...
Each of the components can be run from your IDE as a Java application or using Maven:

`mvn clean package`

Alternatively, the Jar files created (in the `\target` directory of each project), may be run from separate shells. It is important to first run the EurekaServer, then the two arithmetic microservices (AdditionServer, SubtractionServer), then the WebServer.
